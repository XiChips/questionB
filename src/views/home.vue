          ...
          <div class="content">
            ...
            <div class="area">
              <a-form :model="formState" autocomplete="off" @finish="onFinish" @finishFailed="onFinishFailed">
                ...
                <div class="line2">
                  <a-form-item label="购进指标">
                    ...
                    <a-input v-model:value="inputs.value4" placeholder="输入金额/数量" style="width: 250px;margin-left: 10px;"></a-input>
                  </a-form-item>
                  <a-form-item label="纯销指标">
                    <a-select v-model:value="inputs.value5" placeholder="" style="width: 88px" :options="options3" :filter-option="filterOption" @focus="handleFocus" @blur="handleBlur" @change="handleChange">
                      <template #suffixIcon>
                        <down-outlined style="width: 15px;height: 15px;color: rgb(78,139,254);" />
                      </template>
                    </a-select>
                    <a-input v-model:value="inputs.value6" placeholder="输入金额/数量" style="width: 250px;margin-left: 10px;"></a-input>
                  </a-form-item>
                </div>
                <div class="line2">
                  ...
                  <a-form-item label="签订时间">
                    <a-date-picker placeholder="" show-time v-model:value="value8" style="width:350px" />
                  </a-form-item>
                </div>
              </a-form>
              <div class="line2">
                <a-form-item label="购进渠道">
                  <a-select v-model:value="inputs.value9" placeholder="" style="width: 88px" :options="options5" :filter-option="filterOption" @focus="handleFocus" @blur="handleBlur" @change="handleChange">
                    <template #suffixIcon>
                      <down-outlined style="width: 15px;height: 15px;color: rgb(78,139,254);" />
                    </template>
                  </a-select>
              ...
              <div style="padding-left: 3vw;">
                <a-table v-if="this.inputs.value9" :dataSource="data" :columns="columns" :pagination="false" />
              </div>
            ...
            <div class="area" style="">
              <div class="areaType">
                <span>产品：</span>
                <a-button style="margin-left: 20px;">选择产品</a-button>
                <a style="margin-left: 20px;">美复胶丸 24粒/盒</a>
                <span style="margin-left: 80px;">协议效期：</span>
                <a-range-picker v-model:value="value4" :placeholder="['','']" :format="dateFormat" />
              </div>
              ...
              <a-table :columns="columns3" :data-source="data3" bordered :pagination="false" style="margin-top: 20px;">
                <template #bodyCell="{ column, text, record }">
            ...
            <div class="area">
              <a-tabs v-model:activeKey="activeKey2" type="card">
                <template #rightExtra>
                  <a-button type="primary" style="margin-right: 10px;" @click="addTab">新增</a-button>
                </template>
                <a-tab-pane v-for="(item,id) in tabs" :key="id+1" :tab="'补充协议'+(id+1)">
                  <div style="display: flex;justify-content: space-between;margin: 10px; 0">
                    <span>补充协议：</span>
                    <a-button @click="deleteTab(id)" class="btn">删除</a-button>
                  </div>
                  <textarea></textarea>
                </a-tab-pane>
