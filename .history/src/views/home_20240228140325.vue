              ...
              <a-table :columns="columns3" :data-source="data3" bordered style="margin-top: 20px;">
                <template #bodyCell="{ column, text, record }">
                  ...
                  <template v-if="[ 'col2',  'col4',  'col6', ].includes(column.dataIndex)">
                    <div>
                      <a-select v-model:value="record.key" show-search placeholder="" style="width: 200px" :options="options" :filter-option="filterOption" @focus="handleFocus" @blur="handleBlur" @change="changeCustom"></a-select>
                    </div>
                  </template>
                </template>
              </a-table>
