              ...
              <a-form :model="formState" autocomplete="off" @finish="onFinish" @finishFailed="onFinishFailed">
                ...
                <div class="line2">
                  <a-form-item label="销售区域">
                    ...
                    <a-select v-if="!visible2" v-model:value="inputs.value7c" show-search placeholder="" style="width: 200px" :options="options4" :filter-option="filterOption" @focus="handleFocus" @blur="handleBlur" @change="changeArea"></a-select>
                    <a-button v-if="inputs.value7.length" v-for="item in inputs.value7" style="margin-left: 20px;">{{ item }}</a-button>
                  </a-form-item>
...
export default defineComponent({
  ...
  methods: {
    ...
    changeArea() {
      this.visible2 = true
      this.inputs.value7.push(this.inputs.inputs7c)
      console.log(this.inputs.value7c)
      this.inputs.value7 = Array.from(new Set(this.inputs.value7))
      console.log(this.inputs.value7)
    },
  },
  created() { },
  beforeMounte() { },
