              ...
              <a-form :model="formState" autocomplete="off" @finish="onFinish" @finishFailed="onFinishFailed">
                ...
                <div class="line2">
                  <a-form-item label="销售区域">
                    ...
                    <a-button type="primary" v-if="inputs.value7.length" v-for="(item,id) in inputs.value7" style="margin-left: 20px;"><span @click="deleteItem(id)">{{ item }}
                        <close-outlined /></span></a-button>
                  </a-form-item>
                  <a-form-item label="签订时间">
                    <a-select v-model:value="value" placeholder="" style="width: 200px" :options="options2" :filter-option="filterOption" @focus="handleFocus" @blur="handleBlur" @change="handleChange">
                      <template #suffixIcon>
                        <down-outlined style="width: 15px;height: 15px;color: rgb(78,139,254);" />
                      </template>
                    </a-select>
